...
module ManagementAPI
  module V1
    ...
    private
      ...
      def check_jwt!(jwt)
        begin
          Rails.logger.debug { "security_configuration.fetch(:scopes)\n" }
          Rails.logger.debug { "#{(security_configuration)&.fetch(:scopes).inspect} .fetch(security_scope) .fetch(#{security_scope})\n" }
          Rails.logger.debug { "#{(security_configuration)&.fetch(:scopes)&.fetch(security_scope).inspect}" }
          scope    = security_configuration
            .fetch(:scopes)
            .fetch(security_scope)
