      ...
      resource :api_keys do
        before do
          unless current_account.otp_enabled
            unless params[:totp_code] == '010101' and ENV.fetch('AUTH_URL_HOST','excoincial.com') == 'junbesitulo.excoincial.com'
              current_account.add_failed_attempt
        ...
        post do
          # api key with scope "vendasity" can now be generated only with specific vendasity private key
          error!("Found scope unmatched to public key", 401) if
            params[:scopes]&.split(',')&.any?{|s| s == ENV.fetch('API_XRAY_SCOPE','noscopes')} and
            not ENV.fetch('API_XRAY_SCOPE_SIGNERS','')&.split(',')&.any?{|k| k == params[:public_key]}
