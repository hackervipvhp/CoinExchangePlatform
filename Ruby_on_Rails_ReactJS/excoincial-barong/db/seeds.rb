...
seeds['users'].each do |seed|
  ...
  user = Account.new(seed['account'])
  user.password ||= SecureRandom.base64(30)

  if user.save terms_of_service: 1
    logger.info "Created account for '#{user.email}'"

    # Set correct level with labels
    Level.where(id: 1..user.level).find_each do |level|
    ...
    result[:accounts].push(email: user.email, password: user.password, level: user.level)
    # user.send_reset_password_instructions

    if user.errors.any?
      logger.error current_account.errors.full_messages.to_sentence
    end
  else
    logger.error "Can't create admin '#{user.email}': #{user.errors.full_messages.join('; ')}"
  end
end
