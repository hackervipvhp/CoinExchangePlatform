...
module Matching
  ...
  private
    ...
    def validate!
      Rails.logger.error { "ask (#{@ask.nil?}) and/or bid (#{@bid.nil?}) doesn't exist\nqueue record payload #{@payload.inspect}" } if @ask.nil? or @bid.nil?
    ...
    def create_trade_and_strike_orders
      ...
      ActiveRecord::Base.transaction do
        ...
        begin
          ...
          Rails.logger.error { "@ask.member_id #{@ask.member_id}"}
          Rails.logger.error { "@ask #{@ask.inspect}"}
          Rails.logger.error { "@bid #{@ask.inspect}"}
          Rails.logger.error { "@ask.ask:@ask.member_id\t#{@ask.ask}:#{@ask.member_id}" }
          Rails.logger.error { "@ask.bid:@ask.member_id\t#{@ask.bid}:#{@ask.member_id}" }
          Rails.logger.error { "@bid.bid:@bid.member_id\t#{@bid.bid}:#{@bid.member_id}" }
          Rails.logger.error { "@bid.ask:@bid.member_id\t#{@bid.ask}:#{@bid.member_id}" }
          report_exception e
          raise e
        end
    ...
    def publish_trade
      ...
      [@ask, @bid].each do |order|
        ...
        order.calculate_reward if event == 'order_completed'
    ...
    def strike(trade, order, outcome_account, income_account)
      ...
      begin
        ...
        report_exception e
        Rails.logger.error { "#{outcome_account.inspect}\n#{income_account.inspect}" }
        raise e
      end
