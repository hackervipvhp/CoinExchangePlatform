...
module WalletService
  class Geth < Base
    ...
    DEFAULT_ETH_FEE = { gas_limit: 21_000, gas_price: 8_000_000_000 }.freeze

    DEFAULT_ERC20_FEE = { gas_limit: 90_000, gas_price: 8_000_000_000 }.freeze
    ...
    def deposit_collection_fees(deposit, options = {})
      ...
      if eth_fees_value * 1e-18  > load_balance(destination_address, Currency.find('eth'))
    ...
    def collect_erc20_deposit!(deposit, options = {})
      ...
      spread_hash.map do |address, amount|
        ...
        spread_amount = deposit.currency.base_factor * client.load_balance_of_address(pa.address, deposit.currency)
