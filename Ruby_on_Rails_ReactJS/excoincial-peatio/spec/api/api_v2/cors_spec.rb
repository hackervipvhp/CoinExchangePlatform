...
describe APIv2::Members, type: :request do
  ...
  def check_cors(response)
    expect(response.headers['Access-Control-Allow-Origin']).to eq( ENV.fetch('URL_SCHEME') + '://excoincial.com')
  ...
  before { ENV['API_CORS_ORIGINS'] = ENV.fetch('URL_SCHEME') + '://excoincial.com' }
