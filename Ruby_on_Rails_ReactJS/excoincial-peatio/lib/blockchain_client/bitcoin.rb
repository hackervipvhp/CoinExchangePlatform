...
module BlockchainClient
  class Bitcoin < Base
    ...
    def load_balance!(address, currency, query_options = {}, exclude_addresses = [])
      json_rpc(:listunspent,
        ( query_options.present? ?
          [
            1,
            10_000_000,
            ( address.present? ? [address] : [] ),
            true, query_options
            # query_options allows ignoring amounts hard to be used, e.g. {minimumAmount:"0.00005"}
          ] : [
            1,
            10_000_000,
            ( address.present? ? [address] : [] )
        ...
        .fetch('result')
        .sum { |vout| (
            (exclude_addresses.present? and
              exclude_addresses&.include? vout['address']) ?
            0.to_f :
            vout['amount'].to_f
          )
        }
        # will exclude addresses (e.g. hot warm cold wallets) from summing listunspent vouts
    end
