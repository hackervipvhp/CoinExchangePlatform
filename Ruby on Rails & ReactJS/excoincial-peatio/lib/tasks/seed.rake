...
namespace :seed do
  ...
  desc 'Add faq settings'
  task faq_settings: :environment do
    FaqSetting.transaction do
      YAML.load_file(Rails.root.join('config/seed/faq.yml')).each do |hash|
        next if FaqSetting.exists?(question: hash.fetch('question'))
        FaqSetting.create!(hash)
      end
    end
  end
end
