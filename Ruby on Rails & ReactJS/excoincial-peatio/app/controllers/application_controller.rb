...
class ApplicationController < ActionController::Base
  ...
  before_action :set_language, :set_gon, :valid_user
  around_action :share_user

private

  def valid_user
    return if request.path == "/setmode/nightmode" || request.path == "/setmode/daymode" || request.path == "/logout" || request.path.split("/").last == "approve_login" || request.path.split("/").last == "disable_account" || request.path == "/coins-info" || request.path == "/swagger" || request.path == "/login_privacy" || request.path == "/login_terms_cond" || request.path == "/" || request.path == "/login_supportfooter"
    if session["valid_user"] == false
      if request.format.json?
        render json: {valid_user: false}
      else
        begin
          @login_history = RestClient.get(
            "#{ENV.fetch('BARONG_DOMAIN')}/users/api/v1/accounts/last_login?access_token="+current_user.auth('barong').token
          )
          @login_history =  JSON.parse @login_history
          time = time_cal(@login_history["updated_at"].to_time,Time.current.to_time)
          flash[:notice] = "You last logged in #{time} on #{@login_history["updated_at"].to_time.strftime("%d %b %Y")} at #{@login_history["updated_at"].to_time.utc.strftime("%H:%M")} UTC from #{@login_history["ip_address"]}"
          render "welcome/login_alert", :layout => "funds"
        rescue
          flash[:error] = "Your account was either disabled on your request, blocked by our officer, encountered an unexpected issue or guarded against suspicios activity. Please contact support."
          render "welcome/login_alert", :layout => "landing"
        end
      end
    end
  end
  ...
  def current_user
    Rails.logger.debug{ "session of #{:member_id} session['valid_user'] #{session["valid_user"]} #{session[:member_id].inspect}" }
